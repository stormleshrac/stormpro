...
class MoodleClient(object):
    def __init__(self, user,passw,host='',repo_id=4,token='',proxy:ProxyCloud=None):
        self.username = user
        self.password = passw
    ...
    def getUserData(self):
        try:
            tokenUrl = self.path+'login/token.php?service=moodle_mobile_app&username='+parse.quote(self.username)+'&password='+parse.quote(self.password)
    ...
    def delete(self,enlace):
        try:
            ...
            filename = splited[-1]
            itemid = splited[-2]
            parsedFileName = parse.unquote(filename)
            payload = {
            'sesskey':sesskey,
            'client_id':client_id,
            'filepath':file_path,
    ...
    def upload_file_perfil(self, file, progressfunc=None, args=(), tokenize=False):
        ...
        resp2 = self.session.post(post_file_url, data=monitor,
                                  headers={"Content-Type": "multipart/form-data; boundary=" + b}, proxies=self.proxy)
        of.close()
